...
int revDeleteRel_By_RelId(long revRelId) {
    int revDelStatus = -1;
    ...
    char *sql = "DELETE FROM REV_ENTITY_RELATIONSHIPS_TABLE WHERE REV_RELATIONSHIP_ID = ?;";

    int rc = sqlite3_prepare(db, sql, -1, &stmt, 0);
    sqlite3_bind_int64(stmt, 1, revRelId);

    if (rc != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", sqlite3_errmsg(db));

        __android_log_print(ANDROID_LOG_ERROR, "MyApp", "ERR revDeleteRel_By_RelId %s", sqlite3_errmsg(db));
    } else {
        // commit
        sqlite3_step(stmt);
        revDelStatus = 1;
...
int deleteRevRel_By_Unset_Subject_Target_GUID(long revUnsetRelEntityGUID, long revUnsetRelEntityRemoteGUID) {
    ...
    int rc = sqlite3_prepare(db, sql, -1, &stmt, 0);
    sqlite3_bind_int64(stmt, 1, revUnsetRelEntityGUID);
    sqlite3_bind_int64(stmt, 2, revUnsetRelEntityRemoteGUID);
