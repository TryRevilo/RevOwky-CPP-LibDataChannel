...
const requestCameraPermission = () => {
  try {
    PermissionsAndroid.requestMultiple([
      ...
      let dataChannelInit = RevWebRTCReactModule.revInitDataChannel("revLocal", revTarget);

      console.log(`>>> dataChannelInit >> ${dataChannelInit}`);

      let revSendMessageStatus = RevWebRTCReactModule.revSendMessage(revTarget, "Hello World!");

      let revGetTestStr = RevWebRTCReactModule.revGetTestStr("revKey");

      console.log(`>>> revGetTestStr ${revGetTestStr}`);

      console.log(`>>> revSendMessageStatus ${revSendMessageStatus}`);
...
const SiteContainer = () => {
  let revInitWSStatus = RevWebRTCReactModule.revInitWS("192.168.191.220", "revLocal");

  console.log(`>>> revInitWSStatus ${revInitWSStatus}`);

  let revSetTestStr = RevWebRTCReactModule.revSetTestStr("revKey", ">>> Rev Val >>>");
